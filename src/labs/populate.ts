...
async function main() {
	...
	console.log(`Uploading ${formattedProducts.length} products in batches of ${BATCH_SIZE}...`);
	
	for (let i = 0; i < formattedProducts.length; i += BATCH_SIZE) {
		const batch = formattedProducts.slice(i, i + BATCH_SIZE);
		console.log(`Uploading batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(formattedProducts.length / BATCH_SIZE)} (${batch.length} products)...`);
		await vectorIndex.upsert(batch);
	}
	
	console.log(`Successfully uploaded ${formattedProducts.length} products to vector index.`);
}
